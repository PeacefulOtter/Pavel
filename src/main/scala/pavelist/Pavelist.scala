...
class PavelEmpty[O]( list: List[O] ) extends Pavelist[O, O]
{
    ...
    override def conc(): ConcPavelist[O, O] = ConcPavelEmpty( list )
}

class PavelNonEmpty[O, T](list: List[O], queue: ApplyQueue[O, _, T] ) extends Pavelist[O, T]
{
    ...
    override def conc(): ConcPavelist[O, T] = ConcPavelNonEmpty( list, queue )
}
